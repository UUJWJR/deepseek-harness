...
紧凑工具调用胶囊。它绑定持久化 `compactMode` 设置，并在该设置开启时注册一个逐调用的紧凑工具 `ConversationNodeDefinition` 及其 keyed 胶囊渲染器，使每个工具调用在完整调用树旁也渲染为一行胶囊（名称 + 完成标记）。设置 General 区域中的紧凑模式复选框行拥有该开关。它不注册任何面向模型的界面。
