...
Browser file tree for the filesystem. It registers a Files footer action in the sidebar that opens a modal file tree — breadcrumbs, a directory listing, a show-hidden toggle, and multi-select bulk delete — driven by the host `ctx.remote.files` Remote. It registers no model-facing surface.
...
- **No move or drag** — only browse, select, and delete ship; move and drag-to-move are deferred.
