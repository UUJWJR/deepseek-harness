 ...
 * selection, a show-hidden toggle, and a bulk-delete action. Navigation and
 * selection are component-local; entries arrive through the injected list and
 * deletePath verbs. Ancestor folding runs client-side before delete.
 ...
 * @param props - the starting directory, the list/delete verbs, and the translator.
 * @returns the breadcrumbs, toolbar, and entry list.
 */
export function FilesPanel({ root, list, deletePath, t }: FilesPanelProps) {
  ...
  const performDelete = useCallback(async () => {
    ...
    setEntries(value)
    setLoading(false)
  }, [selection, deletePath, list, path])
            ...
            <li key={entry.path}>
              <div className={css.row}>
                ...
                {entry.type === 'directory'
                  ? (
                    <button type="button" className={css.name} data-testid="files-dir" onClick={() => { openPath(entry.path) }}>
                      {entry.name}/
                    </button>
                  )
