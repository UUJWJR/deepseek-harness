...
`publish` copies the source file (up to 50 MiB) into a per-report subdirectory named by its opaque id and appends a metadata record; re-publishing the same source returns the existing report with `deduplicated: true`. `list` filters by an optional tag; `tags` returns distinct tags in first-seen order; `read` returns one report and its copied file content, throwing `REPORT_NOT_FOUND` for a missing id.
