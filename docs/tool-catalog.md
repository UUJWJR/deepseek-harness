...
Delete a file or an empty directory. Fails if the directory is not empty.
...
Move a file or directory to a destination path. Moving a directory into its own descendant fails.
